...
export default function EditTransactionPage() {
  ...
  useEffect(() => {
    ...
    api<{ data: Transaction }>(`/api/workspaces/${workspaceId}/transactions/${id}`)
      .then((r) => {
        const t = (r as { data: Transaction }).data;
        setTransaction(t);
  ...
  useEffect(() => {
    ...
    Promise.all([
      api<{ data: { accounts: Account[] } }>(`/api/workspaces/${workspaceId}/accounts`),
      api<{ data: Category[] }>(`/api/workspaces/${workspaceId}/categories`),
    ]).then(([accRes, catRes]) => {
      const accData = accRes as { data?: { accounts?: Account[] } };
      const catData = catRes as { data?: Category[] };
